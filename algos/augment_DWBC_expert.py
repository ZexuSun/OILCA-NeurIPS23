...
class DWBC(object):
    ...
    def train(self, state_e, action_e, next_state_e, state_o, action_o, shift, scale):
        ...
        state_e_in, action_e_in = state_e.clone(), action_e.clone()    
...
#----------------------------
        if self.total_it % self.d_update_num == 0 and self.aug_steps != 0:
            ...
            for aug_step in range(self.aug_steps):
                ...
                state_e = torch.cat([state_e_in, aug_state_e], axis=0).detach()
                action_e = torch.cat([action_e_in, aug_action_e], axis=0).detach()

            #sample
            e_idx = np.random.randint(0, len(state_e), size=self.batch_size)
            state_e = state_e[e_idx]
            action_e = action_e[e_idx]

            if self.eta_change == True:
                log_pi_e_aug = self.policy.get_log_density(state_e, action_e)
                log_pi_e_aug_clip = torch.clip(log_pi_e_aug, LOG_PI_NORM_MIN, LOG_PI_NORM_MAX)
                log_pi_e_aug_norm = (log_pi_e_aug_clip - LOG_PI_NORM_MIN) / (LOG_PI_NORM_MAX - LOG_PI_NORM_MIN)
                d_e_aug = self.discriminator(state_e, action_e, log_pi_e_aug_norm.detach())
                
                
                d_e_aug_clip = torch.squeeze(d_e_aug).detach()
                d_e_aug_clip[d_e_aug_clip <= 0.5] = 0.0
                d_e_aug_clip[d_e_aug_clip > 0.5] = 1.0
                # print(torch.sum(d_e_aug_clip))
                self.eta = torch.sum(d_e_aug_clip) / self.batch_size
